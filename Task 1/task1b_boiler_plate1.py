#!/usr/bin/env python3
"""
Task1B: Bad Fruit Detection & TF Publisher
------------------------------------------
Detect bad (greyish-white) fruits on the conveyor and publish their 3D positions
as TF frames named <TEAM_ID>_bad_fruit_<id>.
Also visualize contours and bounding boxes in real-time.
"""
...
TEAM_ID = "KR25"
TRAY_POLYGON = Polygon([(57,230), (368,230), (320,384), (0,385)])  # same as provided
...
class Task1BNode(Node):
    ...
    def image_callback(self, color_msg: Image, depth_msg: Image):
        ...
        # Detect greyish/white (bad) fruits
        hsv = cv2.cvtColor(color_cv, cv2.COLOR_BGR2HSV)
        # Low saturation & high value => greyish/white
        ...
        for cnt in contours:
            ...
            # Publish TF frame for this fruit
